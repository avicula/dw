...
// @description     Adds html to textarea, allows icon browsing on free accounts, automatically views all icons, and clicks through warnings.
// @author          SimplerUser (edited for ff users by janekptacijarabaci), leitmotif
// @version         5.0
...
function detect_txtbx() {
    ...
    if (txtbx !== null) {
        ...
        var htmlcode = [
            ...
            '<span style="font: bold 1rem sans-serif;"></span>',
            '<ul></ul>',
      ...
      if (txtbx = document.getElementById("draft")) {
        ...
        function TextButtonClickAction() {
            ...
            var end = txtbx.selectionEnd;
            var sel = txtbx.value.substring(start, end);
            var toAdd = '<span style="font: 1rem monospace;"></span>';
            var curPos = doGetCaretPosition(txtbx);
            var curVal = txtbx.value;
            var htmlLength = toAdd.indexOf("</");
            var res = toAdd.replace("></", ">" + sel + "</");
...
$(document).ready(function() {
    ...
    $('head').append(`<style type="text/css">
        ...
        #iconselector_icons_list li { display:inline-block; margin:.25em; padding:.25em; width: 110px; text-align:center; border: rgba(204,204,204,.5) 1px solid; overflow:hidden; }
        ...
        #comments #qrformdiv #qrform { padding: 2em !important; border: rgba(204,204,204,.5) 1px solid; }
        #qrformdiv { max-width: 900px !important; margin: 1.5em 0; padding: 0 !important; border: 0; }
        #qrformdiv .qr-meta { width: 75px; max-width: 100% !important; margin-right: 5px; display: block !important; float: left; }
        #qrformdiv .qr-meta * { height: auto !important; width: 75px !important; display: block !important; }
        #qrformdiv .qr-body { width: calc(100% - 80px); display: inline-block; }
        #qrformdiv .qr-markup-type * { margin-right: 5px; }
        #qrformdiv #submitmoreopts { display: none !important; }
        #qrformdiv .block-icon a::after, #qrformdiv .block-icon button::after { content: none !important; }
        #qrformdiv #randomicon { margin: 0 !important; }
        </style>`);
    ...
    if ($('#lj_userpicselect').length) {
        ...
        $('.block-icon a').wrap('<button type="button" id="lj_userpicselect" data-iconbrowser-metatext="false" data-iconbrowser-smallicons="false"></button>');
